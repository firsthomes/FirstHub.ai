import type { DayLog, FoodEntry, WeightEntry } from '../types'
import { getDay, saveDay, saveWeight } from './storage'
...
function matchFood(item: string, qtyStr: string, unit: string, notes: string): Macros | null {
  ...
  if (/^honey|^.*honey/.test(lower) && !/yogurt/.test(lower)) {
    let multiplier = qty
  ...
  if (/lcm/.test(lower)) {
    return { name: 'LCM bar', calories: 90, protein: 0, carbs: 18, fat: 2, quantity: qty }
  }
  ...
  if (/avocado/.test(lower)) {
    return { name: 'Avocado', calories: 320, protein: 4, carbs: 16, fat: 28, quantity: qty }
  }

  if (/bagel/.test(lower)) {
    return { name: 'Bagel', calories: 250, protein: 9, carbs: 49, fat: 2, quantity: qty }
  }
  ...
  if (/burrito bowl|guzman/.test(lower)) {
    if (/chip/.test(lower)) {
      return { name: 'GYG Small Chips', calories: 300, protein: 4, carbs: 35, fat: 16, quantity: qty }
    }
    return { name: 'GYG Burrito Bowl', calories: 650, protein: 40, carbs: 70, fat: 22, quantity: qty }
  }

  if (/small chips|chips/.test(lower)) {
    return { name: 'Small Chips', calories: 300, protein: 4, carbs: 35, fat: 16, quantity: qty }
  }

  if (/garlic bread/.test(lower)) {
    return { name: 'Garlic Bread', calories: 280, protein: 6, carbs: 32, fat: 14, quantity: qty }
  }

  if (/donut|long john/.test(lower)) {
    return { name: 'Donut', calories: 300, protein: 4, carbs: 38, fat: 15, quantity: qty }
  }

  if (/ice cream/.test(lower)) {
    return { name: 'Ice Cream', calories: 250, protein: 4, carbs: 28, fat: 14, quantity: qty }
  }

  if (/chocolate croissant|croissant/.test(lower)) {
    return { name: 'Chocolate Croissant', calories: 280, protein: 5, carbs: 32, fat: 15, quantity: qty }
  }
  ...
  if (/digestive/.test(lower)) {
    return { name: 'Digestive Biscuit', calories: 70, protein: 1, carbs: 10, fat: 3, quantity: qty }
  }

  if (/oat bar/.test(lower)) {
    return { name: 'Oat Bar', calories: 200, protein: 4, carbs: 30, fat: 7, quantity: qty }
  }

  if (/maximus|sports drink|gatorade/.test(lower)) {
    return { name: 'Sports Drink', calories: 200, protein: 0, carbs: 50, fat: 0, quantity: qty }
  }

  if (/mango solo|energy drink|solo/.test(lower)) {
    return { name: 'Energy Drink', calories: 140, protein: 0, carbs: 35, fat: 0, quantity: qty }
  }

  if (/chicken roll/.test(lower)) {
    return { name: 'Chicken Roll', calories: 350, protein: 18, carbs: 30, fat: 16, quantity: qty }
  }

  if (/ham.*cheese|toasted sourdough|toastie/.test(lower)) {
    return { name: 'Ham Cheese Toastie', calories: 450, protein: 22, carbs: 38, fat: 22, quantity: qty }
  }

  if (/curry/.test(lower)) {
    return { name: 'Curry meal', calories: 700, protein: 30, carbs: 70, fat: 30, quantity: qty }
  }

  if (/chinese/.test(lower)) {
    return { name: 'Chinese meal', calories: 700, protein: 30, carbs: 85, fat: 25, quantity: qty }
  }

  if (/tinned tomato|tomato/.test(lower)) {
    return { name: 'Tinned Tomatoes', calories: 80, protein: 4, carbs: 16, fat: 0, quantity: qty }
  }

  if (/carrot/.test(lower)) {
    return { name: 'Carrot', calories: 30, protein: 1, carbs: 7, fat: 0, quantity: qty }
  }

  if (/rokeby/.test(lower)) {
    return { name: 'Rokeby + YoPro 25g', calories: 220, protein: 25, carbs: 18, fat: 4, quantity: qty }
  }

  if (/steamed chicken/.test(lower)) {
    return { name: 'Steamed Chicken Bowl', calories: 600, protein: 45, carbs: 70, fat: 12, quantity: qty }
  }
...
function shouldImportFood(row: CSVRow): boolean {
...
function shouldImportWeight(row: CSVRow): boolean {
  const time = (row.measurement_time || '').toLowerCase()
  return time.includes('morning')
}

function inferTraining(row: CSVRow): 'upper' | 'lower' | 'rest' | null {
  const timing = (row.timing || '').toLowerCase()
  if (timing.includes('pre-gym') || timing.includes('post-gym')) return 'upper'
  return null
}

function inferActivity(timing: string): 'walk' | 'run' | 'motocross' | 'golf' | 'active-rest' | null {
  const t = timing.toLowerCase()
  if (t.includes('motocross')) return 'motocross'
  if (t.includes('riding')) return 'motocross'
  return null
}
...
export function importCSV(text: string): ImportSummary {
  ...
  function getOrLoadDay(date: string): DayLog {
    let day = dayMap.get(date)
    if (!day) {
      day = getDay(date)
      day = {
        ...day,
        entries: day.entries.filter(e => !e.foodId.startsWith('imported-')),
      }
      dayMap.set(date, day)
    }
    return day
  ...
  for (const row of rows) {
    if (row.log_type === 'weight') {
      ...
      if (!isNaN(w) && shouldImportWeight(row)) {
        saveWeight({ date: row.date, weight: w })
        const day = getOrLoadDay(row.date)
        day.weight = w
        weightCount++
      }
      ...
      if (!day.training) {
        const tr = inferTraining(row)
        if (tr) day.training = tr
      }
      const act = inferActivity(row.timing || '')
      if (act && !day.activity.includes(act)) {
        day.activity.push(act)
      }
    }
  }
  ...
  for (const day of dayMap.values()) {
    if (day.weight != null) weightEntries.push({ date: day.date, weight: day.weight })
  }
